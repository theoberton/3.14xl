...
function Ipfs() {
  ...
  return (
    <div className="App">
      ...
      <br />
      <pre>{files && JSON.stringify([...files].map(file => ({
       ...
       <br />
       <div style={{ display: 'flex', gridGap: '1rem', height: '16rem', margin: '1rem', flexWrap: 'wrap'  }}>{previews?.map(p => <img style={{ objectFit: 'cover', height: '100%'}} key={p.cid as React.Key} src={`https://ipfs.io/ipfs/${p.cid}`} /> )}</div>
    </div>
  )
}
